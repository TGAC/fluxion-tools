...
public class JSONUtils {
  ...
  public static JSONObject JSONObjectResponse(Map.Entry<String, String>... entries) {
    JSONObject response = new JSONObject();
    try {
      for (Map.Entry<String, String> entry : entries) {
        response.put(entry.getKey(), URLEncoder.encode(entry.getValue(), "UTF-8"));
      }
    }
    catch (UnsupportedEncodingException e) {
      e.printStackTrace();
  ...
  public static JSONObject JSONObjectResponse(Map<String, Object> jsonMap) {
    JSONObject response = new JSONObject();
    for (String key : jsonMap.keySet()) {
      ...
      if (o != null) {
        if (o instanceof String) {
          try {
            response.put(key, URLEncoder.encode((String)o, "UTF-8"));
          }
          catch (UnsupportedEncodingException e) {
            e.printStackTrace();
          }
