...
public class SpiProcessor extends AbstractProcessor {
  ...
  public boolean process(Set<? extends TypeElement> typeElements,
    ...
    Map<String, PrintWriter> outputs = new HashMap<String, PrintWriter>();
    ...
    for (Element element :
      ...
      if (result != null) {
        String spiName = findSpiAnnotation(typeElement).toString();
        if (spiName == null) {
          processingEnv.getMessager().printMessage(
          ...
          PrintWriter pw = outputs.get(spiName);
        ...
        processingEnv.getMessager().printMessage(
            Diagnostic.Kind.ERROR,
            "Classes annotated with " + ServiceProvider.class.getName() + " " +
